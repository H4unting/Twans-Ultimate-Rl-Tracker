...
const predicates = {
  dateFrom: (g, v) => !v || (parseDisplayDate(g.date) >= parseDisplayDate(v)),
  dateTo: (g, v) => !v || (parseDisplayDate(g.date) <= parseDisplayDate(v)),
  playlist: (g, v, gameId) => modeMatchesPlaylist(g, v, gameId),
  session: (g, v) => !v || parseInt(g.session, 10) === parseInt(v, 10),
...
export function getUniqueSessions(games, gameId = state.activeGame) {
  ...
  return [...sessions.values()].sort((a, b) => parseInt(b.session, 10) - parseInt(a.session, 10));
}
