...
import { getGameMeta, GAME_IDS, getRankDiff, getRankValue, getGameRankStart } from './games.js';
...
export function exportGamesCSV(games, playerName, gameId = GAME_IDS.ROCKET_LEAGUE) {
  ...
  const rows = games.map(g => (isVal
    ? [g.match, g.date, g.session, g.mode, g.result, g.kills ?? g.goals, g.deaths, g.valAssists ?? g.assists, g.agent ?? '', g.map ?? '', getGameRankStart(g, gameId), getRankValue(g, gameId), getRankDiff(g, gameId), (g.tags || []).join('; '), g.notes || '']
    : [g.match, g.date, g.session, g.mode, g.result, g.goals, g.assists, g.saves, getGameRankStart(g, gameId), getRankValue(g, gameId), getRankDiff(g, gameId), (g.tags || []).join('; '), g.notes || '']
  ).map(csvEscape).join(','));
...
export function printWeeklyReport(report, playerName, coachLines = [], gameId = GAME_IDS.ROCKET_LEAGUE) {
  ...
  const coachBlock = coachLines.length
    ? `<h3>Coach Notes</h3><ul>${coachLines.map(l => `<li>${escapeHtml(l.text)}</li>`).join('')}</ul>` : '';

  const safeName = escapeHtml(playerName);
  const safeLabel = escapeHtml(report.label);
  const topMistakeBlock = report.topMistake
    ? `<h3>Top ${isVal ? 'Leak' : 'Mistake'}</h3><p>${escapeHtml(report.topMistake[0])} (${escapeHtml(report.topMistake[1])}×)</p>`
    : '';

  win.document.write(`<!DOCTYPE html><html><head><title>${safeName} — Weekly Report</title>
    <style>
    ...
    <div class="meta">${safeName} · ${safeLabel}</div>
    <div class="grid">
    ...
    ${topMistakeBlock}${vsBlock}${coachBlock}
