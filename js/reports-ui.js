...
import { getGoalProgress, getActiveGoals, mergeActiveGoals, getFocusTagOptions } from './goals.js';
...
function renderGoalsEditor(onGoalsSave) {
  ...
  el.innerHTML = `
    <div class="goals-form-grid">
      <div class="form-group"><label>${meta.rankLabel} Target</label>
        <input type="number" id="goal-mmr" min="0" value="${g.mmrTarget || ''}" placeholder="${isVal ? 'e.g. 50' : 'e.g. 850'}"></div>
      <div class="form-group"><label>${isVal ? 'Matches / Week' : 'Games / Week'}</label>
