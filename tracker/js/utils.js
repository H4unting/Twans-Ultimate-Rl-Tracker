...
import { DEFAULT_GAME, getGameModule, getDefaultMode, RL, sessionGroupKey } from './games/registry.js';
import {
...
export function groupSessionsForHistory(games) {
  const map = new Map();
  games.forEach(g => {
    const key = sessionGroupKey(g);
    const sn = parseInt(g.session, 10) || 1;
    if (!map.has(key)) {
      map.set(key, { sessionNum: sn, games: [], firstDate: g.date, lastDate: g.date });
    }
  ...
  return [...map.values()]
    ...
    .sort((a, b) => b.sessionNum - a.sessionNum || (b.lastDate > a.lastDate ? 1 : -1));
}
