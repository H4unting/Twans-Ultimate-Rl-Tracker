...
const built = path.join(__dirname, '..', 'dist', 'Twans-Tracker-Bridge.exe');
const dest = path.join(__dirname, '..', '..', 'Twans-Tracker-Bridge.exe');
...
try {
  ...
  const alt = path.join(path.dirname(dest), 'Twans-Tracker-Bridge-new.exe');
  try {
    copyWithRetry(built, alt);
    console.warn('Could not overwrite locked exe. Saved as:', alt);
    console.warn('Close Twans-Tracker-Bridge.exe, then rename the new file.');
  } catch {
